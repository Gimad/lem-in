...
/*   Updated: 2020/11/11 14:06:46 by Gimad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_link *find_link, t_path *tmp_path)
{
	...
	find_link->on = 0;
	find_link->in_way = 0;
	hash_key = hash(lem->nb_rooms, tmp_path->forward->name);
	find_room = lem->rooms[hash_key];
	while (ft_strcmp(tmp_path->forward->name, find_room->name) != 0)
